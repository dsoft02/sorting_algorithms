...
/**
 * swapitem - swap the nodes themselves.
 * @current: pointer.
 * @current_old: pointer.
 * @list: doubly linked list
 */
void swapitem(listint_t *current, listint_t *current_old, listint_t **list)
{
	...
	do {
		while (check->next)
		{
			if (check->n > check->next->n)
				swapitem(check->next, check, list);
			else
				check = check->next;
		}
		last = check;
		while (check->prev != first)
		{
			if (check->n < check->prev->n)
				swapitem(check, check->prev, list);
			else
				check = check->prev;
		}
		first = check;
